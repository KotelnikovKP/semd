    ...
    <div class="container">
        <div class="row">
            <div class="col-lg-12">
                ...
                <h1 class="my-3">Лабораторные тесты</h1>

                <div class="row mt-3">
                    <div class="col-md-12">
                        <div class="md-form mb-2">
                            <form class="input-group search-form">
                                <input type="text" class="form-control" name="q" placeholder="Поиск по тестам" v-model="q">
                ...
                <table class="table table-striped table-bordered" id="semds_table">
                    ...
                    <tbody>
                        <tr v-for="semd_test in semd_tests" :key="semd_test.id">
                            <td>{{ Intl.DateTimeFormat().format(Date.parse(semd_test.test_time)) }}</td>
                            <td>{{ semd_test.id }}</td>
                            <td>{{ semd_test.semd_id }}</td>
                            <td>{{ semd_test.patient_name }}</td>
                            <td>{{ semd_test.laboratory_test_name }}</td>
                            <td>{{ semd_test.value }}</td>
    ...
    methods: {
        async fetchPage(p) {
            try {
                ...
                let response = await this.$axios.get(`/api/v1/semd_test?q=${this.q}&page=${p}`);
                this.semd_tests = response.data.result;
